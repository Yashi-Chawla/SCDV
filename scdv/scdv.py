...
class SCDV:
    def __init__(self, init_vector_type="word2vec_sg", num_clusters=10, vector_size=100, use_ngram_training=False, epochs=5):
    ...
    def init_idf_scores(self, corpus):
        ...
        tfidf.fit(corpus)
        self.idf_scores = tfidf.idf_

    def get_cluster_word_vectors(self):
        for word_id, word in tqdm(enumerate(self.vocabulary)):
            init_word_vector = self.init_vector_model.wv[word]
            cluster_word_vector = list()
            for cluster_id in range(self.num_clusters):
                word_cluster_probability = self.cluster_assignment_probabilities[word_id][cluster_id]
                cluster_word_vector.append(init_word_vector * word_cluster_probability)
            cluster_word_vector = np.concatenate(cluster_word_vector, axis=0)
            cluster_word_vector = cluster_word_vector * self.idf_scores[word_id]
            self.word_vectors[word] = np.asarray(cluster_word_vector)
    ...
    def get_word_vector(self, word):
        if word in self.word_vectors:
            ...
            word = f'<{word}>'
            vector = np.zeros(self.vector_size * self.num_clusters)
            for n in range(3, 7):
    ...
    def fit(self, corpus):
        ...
        logging.info("Obtaining IDF scores for vocabulary")
        self.init_idf_scores(corpus)
        
        logging.info("Fitting Gauassian Mixture Model")
        self.gmm.fit(self.init_vector_model.wv.vectors)
        self.cluster_assignment_probabilities = self.gmm.predict_proba(self.init_vector_model.wv.vectors)

        logging.info("Obtaining cluster word vectors")
        self.get_cluster_word_vectors()
